...
const EditTodo = ({ todo }: { todo: todoTypes }) => {
  ...
  // Wrap the edit action to refresh page after success
  const wrappedEdit = async (formData: FormData) => {
    await edit(formData)
    startTransition(() => {
      router.refresh()
    })
  ...
  return (
    <div className="flex justify-between">
      ...
      <div>
        {editTodo ? (
          <Form action={wrappedEdit} onSubmit={handleSubmit} className="flex justify-between">
            <Input name="inputId" type="hidden" value={todo.id} />
            <div className="flex">
              <Input name="inputTitle" type="text" placeholder="Edit Todo..." />
              <Button
                type="submit"
                text={
                  <>
